...
For the minimum undo tablespace size, you have two options. You can either compute the size automatically or set it manually.
...
3.  From the **Administration** menu, go to the **Storage** option and select **Automatic Undo Management**.
...
1.  On the Automatic Undo Management page in the Analysis Period section, specify the following.
    ...
    **Analysis Time Period**: Select the time period for the analysis. For this lab, leave the default value *Last Seven Days*.
...
You can configure the undo tablespace to have a fixed size. You first set the undo tablespace to its minimum required size in order to handle the workload. Then use the Undo Advisor to set undo tablespace to a fixed size for future long-running queries and Oracle Flashback operations.
...
3.  On the Edit Datafile page, specify the following.
    ...
    **Note**: If the Status of the Tablespace is Offline, then you cannot access it. The Offline status is useful for recovering the data files of the tablespace in event of a failure.
    ...
    The window goes back to the Edit Tablespace page and displays a message to apply the changes.
...
Congratulations! You have successfully completed the workshop on .

In this workshop you learned how to view view and manage the undo data from Oracle EMCC.
